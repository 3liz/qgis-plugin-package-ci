...
def upload_plugin_to_osgeo(
    ...
    try:
        ...
        logger.debug(f"Plugin ID: {plugin_id!r}")
        logger.debug(f"Version ID: {version_id!r}")
...
def release(
    ...
    tx_api_token: Optional[str] = None,
    alternative_repo_url: Optional[str] = None,
    ...
    if not plugin_path:
        raise ValueError("Plugin path required")

    if release_version == "latest":
        parser = ChangelogParser(
            parent_folder=Path(plugin_path).resolve().parent,
            changelog_path=parameters.changelog_path,
        )
        if parser.has_changelog():
    ...
    archive_name = parameters.archive_name(plugin_path, release_version)

    release_tag = cast("str", release_tag or release_version)
    ...
    if dry_run:
        logger.info("Not uploading asset to github because it is a dry run")
        ...
        logger.warning("No github token provided, asset not uploaded")
    ...
    if dry_run:
        logger.info("Not uploading plugin because it is a dry run")
    elif osgeo_username is not None:
        ...
        logger.warning("No osgeo login provided, plugin not uploaded")
